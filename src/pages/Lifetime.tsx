...
import {
  ...
  DETERMINISTIC_SERIES_DEF,
} from '../components/lifetime/DeterministicProjectionChart'
...
import type { SimulationResult, Expense, PensionEstimate, Windfall, UserProfile, MedicalCoverage, MedicalExpense, Account, MonteCarloConfig, RealEstateEvent, Transfer, TaxConfig } from '../types'
...
function ToggleDot({ seriesKey, color, selected }: { seriesKey?: string; color?: string; selected: boolean }) {
  const s = seriesKey ? DETERMINISTIC_SERIES_DEF.find(d => d.key === seriesKey) : null
...
export default function Lifetime() {
  const {
    accounts, profile, expenses, medicalCoverages, medicalExpenses, pensions, windfalls, realEstateEvents,
    monteCarloConfig, taxConfig, minTransactionEUR, transfers,
    liveEurUsdRate,
  } = useAppStore()
