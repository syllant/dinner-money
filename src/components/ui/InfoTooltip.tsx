...
export function InfoTooltip({ text, position = 'center', interactive = false }: {
  text: ReactNode
  ...
  return (
    <span className="relative top-[-3px] inline-flex ml-1 align-middle">
      <span
        ref={triggerRef}
        className="inline-flex items-center justify-center w-3.5 h-3.5 text-[9px] text-gray-400 border border-gray-300 dark:border-gray-600 rounded-full leading-none select-none cursor-help"
        onMouseEnter={() => {
          updatePosition()
          setOpen(true)
        }}
      ...
      <span
        ...
        style={{
          left: coords.left,
          top: coords.top,
          transform: triggerRef.current && triggerRef.current.getBoundingClientRect().top > 96 ? 'translateY(-100%)' : undefined,
        }}
        onMouseEnter={() => {
